...
The site uses the `MDXTwitterTransform.astro` component to scan for these
patterns in blog post content and replace them with proper YouTube embeds. The
component detects special tags using regex and inserts responsive iframes.
...
4. Works with all YouTube URL formats (full youtube.com URLs, youtu.be short
   URLs, or just the video ID)
